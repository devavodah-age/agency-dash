...
const KAIQUE_API = import.meta.env.VITE_KAIQUE_API_URL || 'https://kaique-api-production.up.railway.app'
...
export default function Contacts() {
  ...
  const load = async (silent = false) => {
    ...
    try {
      const res = await fetch(`${KAIQUE_API}/api/leads`)
    ...
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.3)' }}>
            Leads capturados via formulário · Kaique Motos
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          ...
          { label: 'Hoje', value: leads.filter(l => l.created_at?.startsWith(new Date().toISOString().slice(0,10))).length },
          ...
          <div key={label} style={{ background: '#111', border: '1px solid #1a1a1a', borderRadius: '12px', padding: '20px 24px' }}>
            <p style={{ fontSize: '28px', fontWeight: 800, color: '#a78bfa', marginBottom: '4px' }}>{value}</p>
      ...
      <div style={{ background: '#111', border: '1px solid #1a1a1a', borderRadius: '12px', overflow: 'hidden' }}>
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map((lead, i) => (
                <tr key={lead.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #161616' : 'none', transition: 'background .15s' }}
                  onMouseEnter={e => e.currentTarget.style.background = '#161616'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  <td style={{ padding: '14px 16px', fontSize: '13px', fontWeight: 600, color: '#fff' }}>
                    {lead.nome || '—'}
                  </td>
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    <span style={{ background: 'rgba(167,139,250,0.1)', color: '#a78bfa', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>
                      {lead.categoria || '—'}
                    </span>
                  </td>
                  <td style={{ padding: '14px 16px', fontSize: '12px', color: 'rgba(255,255,255,0.35)' }}>
                    {lead.utm_source || '—'}
                  </td>
                  <td style={{ padding: '14px 16px', fontSize: '12px', color: 'rgba(255,255,255,0.35)' }}>
                    {lead.utm_campaign || '—'}
